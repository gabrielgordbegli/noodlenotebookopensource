...
function home() {
  ...
  const createNotebookFormVisibleRef = useRef(createNotebookFormVisible)
  ...
  const createNotebook = async () => {
    ...
    await setDoc(notebookRef, {
      ...
      pdfs: [],
      uid: `${notebookRef.id}`,
      url: 'Paste YouTube Playlist URL'
    })
    const userRef = doc(db, "users", user.uid);
  ...
  const plus = () => {
    setCreateNotebookFormVisible(!createNotebookFormVisible);
  }

  useEffect(() => {
    createNotebookFormVisibleRef.current = createNotebookFormVisible;
  }, [createNotebookFormVisible]);

  React.useEffect(() => {
    window.addEventListener('keydown', (event) => {
      if ((event.key === 'Enter') && (createNotebookFormVisibleRef.current)) {
        document.getElementById("create-notebook-button").click();
      }
    });
  }, []);
        ...
        <div>
          ...
          </div>
          {
            ...
            <div className={styles['create-notebook-form']}>
              <div className={styles['background']} onClick={plus}></div>
              <div className={styles['create-notebook-form-container']}>
                <input autoFocus className={styles['create-notebook-form-input']} type="text" placeholder="Title" onChange={(e) => setNewNotebookName(e.target.value)} />
                <button id="create-notebook-button" className={styles['create-notebook-form-button']} onClick={createNotebook}>Create Notebook</button>
              </div>
            </div>
          }
        </div>
